...
class UserService {
  ...
  async create(userData) {
    ...
    // OAuth Provider hinzufügen falls vorhanden
    ...
    // Mandanten-Ordnerstruktur für neuen User erstellen
    await this.createUserDataStructure(newUser.id);
  ...
  /**
   * Erstellt die Ordnerstruktur für einen neuen User/Mandanten
   */
  async createUserDataStructure(userId) {
    const mandantenBasePath = path.join(__dirname, '../../data/mandanten', userId);
    const templateBasePath = path.join(__dirname, '../../data');
    ...
    try {
      for (const folder of folders) {
        const folderPath = path.join(mandantenBasePath, folder);
        await fs.mkdir(folderPath, { recursive: true });
      }
      
      // Leere firma.md erstellen
      const firmaFile = path.join(mandantenBasePath, 'eigene_firma/firma.md');
      await fs.writeFile(firmaFile, '# Meine Firma\n\nHier können Sie Ihre Firmendaten eintragen.\n', 'utf-8');
      
      // Briefvorlage-Dateien kopieren (Logo, Unterschrift, Template, etc.)
      const briefvorlageFiles = [
        'Logo_Print.png',
        'Unterschrift2.png',
        'template.tex',
        'meta.md',
      ...
      const sourceBriefvorlage = path.join(templateBasePath, 'eigene_firma/briefvorlage');
      const targetBriefvorlage = path.join(mandantenBasePath, 'eigene_firma/briefvorlage');
      
      for (const file of briefvorlageFiles) {
        const sourceFile = path.join(sourceBriefvorlage, file);
        const targetFile = path.join(targetBriefvorlage, file);
        
        try {
          await fs.copyFile(sourceFile, targetFile);
        } catch (err) {
          // Datei nicht vorhanden - überspringen
          console.log(`⚠️  Briefvorlage-Datei nicht gefunden: ${file}`);
        }
      }
      
      // Auch Lexend_Exa Font-Ordner kopieren falls vorhanden
      const fontFolder = path.join(sourceBriefvorlage, 'Lexend_Exa,Urbanist');
      const targetFontFolder = path.join(targetBriefvorlage, 'Lexend_Exa,Urbanist');
      try {
        await fs.cp(fontFolder, targetFontFolder, { recursive: true });
      } catch (err) {
        // Font-Ordner nicht vorhanden - überspringen
      }
      
      console.log(`✅ Mandanten-Struktur für User ${userId} erstellt`);
    } catch (error) {
      console.error(`❌ Fehler beim Erstellen der Mandanten-Struktur:`, error);
      throw error;
    }
  }
